...
function processData(data) {
    ...
    const roundTo100 = (arr) => {
        let rounded = arr.map(Math.round);
        let diff = 100 - d3.sum(rounded);
        while (diff !== 0) {
            let idx = diff > 0 ? rounded.indexOf(d3.min(rounded)) : rounded.indexOf(d3.max(rounded));
            rounded[idx] += Math.sign(diff);
            diff = 100 - d3.sum(rounded);
        }
        return rounded;
    };
    const finalCounts = { race: roundTo100(percentages.race), age: roundTo100(percentages.age), literacy: roundTo100(percentages.literacy) };
...
function transitionToRace() {
    ...
    const labelData = SETTINGS.raceLabels.map((text, i) => ({ text, x: raceScale(i), y: height * 0.25 }));
    updateLabels(labelData);
}

function transitionToAge() {
    const { simulation, nodeSelection, width, height } = sharedD3;
    ...
    simulation
        ...
        .alpha(1).restart();
    nodeSelection.transition().duration(800).attr("fill", d => SETTINGS.raceColors[d.raceGroup]);
    const labelData = SETTINGS.ageLabels.map((text, i) => ({ text, x: ageScale(i), y: height * 0.25 }));
    updateLabels(labelData);
}

function transitionToLiteracy() {
    const { simulation, nodeSelection, width, height } = sharedD3;
    simulation
    ...
    nodeSelection.transition().duration(800)
        .attr("fill", d => (d.literacyGroup === 1) ? SETTINGS.literacyColor : SETTINGS.raceColors[d.raceGroup]);
    const labelData = [
        { text: "93% Alfabetizados", x: width / 2, y: height * 0.25 },
        { text: "7% Não Alfabetizados", x: width / 2, y: height * 0.75 }
    ];
    updateLabels(labelData);
}
    ...
    const observer = new IntersectionObserver((entries) => {
        entries.forEach(entry => {
            if (entry.isIntersecting) {
                ...
                else if (step === 'step-race') transitionToRace();
                else if (step === 'step-age') transitionToAge();
                else if (step === 'step-literacy') transitionToLiteracy();
            }
        });
    }, { threshold: 0.65 });
